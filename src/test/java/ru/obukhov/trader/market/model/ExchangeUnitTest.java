...
class ExchangeUnitTest {
    ...
    void jsonMapping_mapsValue(final String value, final Exchange exchange) throws JsonProcessingException {
        Assertions.assertEquals('"' + value + '"', TestUtils.OBJECT_MAPPER.writeValueAsString(exchange));
    }

    @ParameterizedTest
    @MethodSource("valuesAndExchanges")
    void jsonMapping_createsFromValue(final String value, final Exchange exchange) throws JsonProcessingException {
        Assertions.assertEquals(exchange, TestUtils.OBJECT_MAPPER.readValue('"' + value + '"', Exchange.class));
    }

    @ParameterizedTest
    @MethodSource("valuesAndExchanges")
    void valueOfIgnoreCase(final String value, final Exchange exchange) {
        Assertions.assertEquals(exchange, Exchange.fromValue(value));
    }
