...
public class SimulatorImpl implements Simulator {
    ...
    public SimulateResponse simulate(String ticker, BigDecimal balance, OffsetDateTime from, OffsetDateTime to) {
        ...
        return createSimulateResponse(from, innerTo, ticker);
    }

    private SimulateResponse createSimulateResponse(OffsetDateTime from, OffsetDateTime to, String ticker) {
        return SimulateResponse.builder()
                .currencyBalance(fakeTinkoffService.getBalance())
                .totalBalance(getFullBalance())
                .positions(getPositions())
                .operations(getOperations(from, to, ticker))
                .build();
    }
    ...
    private List<SimulatedOperation> getOperations(OffsetDateTime from, OffsetDateTime to, String ticker) {
        return fakeTinkoffService.getOperations(from, to, ticker).stream()
